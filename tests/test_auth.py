...
_TEST_JWT_SECRET = "test-jwt-secret-for-unit-tests-only-not-production"
...
class TestUserDatabase:
    ...
    def _set_jwt_secret(self, monkeypatch):
        monkeypatch.setenv("JWT_SECRET", _TEST_JWT_SECRET)
        monkeypatch.setenv("ADMIN_PASSWORD", "test-admin-password-for-unit-tests-only")
    ...
    def test_default_admin_exists(self, user_db):
        # Password comes from ADMIN_PASSWORD env var set by the autouse fixture
        admin_user = user_db.authenticate('admin', 'test-admin-password-for-unit-tests-only')
