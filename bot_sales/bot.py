...
class SalesBot:
    ...
    # Inline bullet: "- X - Y - Z" or "• X • Y" flattened by WhatsApp/clipboard paste
    _L1_INLINE_BULLET_RE = re.compile(r"(?:^|[\s:,])\s*[-•*]\s+\S")
    ...
    def _is_structured_list(text: str) -> bool:
        ...
        if "\n" in text:
            ...
            if len(bullet_matches) >= 2:
                return True
        # Inline numbered: "1) tornillos 2) mechas 3) amoladora" — no newlines needed
        inline_matches = SalesBot._L1_INLINE_RE.findall(text)
        if len(inline_matches) >= 2:
            return True
        # Inline bullets: "- mecha 6mm - mecha 8mm - destornillador" pasted flat
        inline_bullet_matches = SalesBot._L1_INLINE_BULLET_RE.findall(text)
        return len(inline_bullet_matches) >= 2
    ...
    def _normalize_list_to_items(text: str, chatgpt_client: "ChatGPTClient") -> str:
        ...
        prompt = (
            ...
            "REGLA IMPORTANTE: conservá TODAS las especificaciones de cada ítem: "
            "material, uso ('para hormigon', 'para metal', 'para drywall'), marca, medidas, color, "
            "presentación y cualquier contexto técnico. "
            "NO descartes palabras como 'para [material/uso]', 'de [material]', medidas, marcas ni especificaciones técnicas. "
            "Sin bullets, sin numerales, sin palabras de relleno. "
            "Si una cantidad no está especificada, asumir 1. "
            "Solo productos reales, sin precios, sin marcas inventadas, sin nada más.\n"
            "Ejemplos:\n"
            "  '- 3 mechas de 6mm para hormigon'  →  '3 mechas de 6mm para hormigon'\n"
            "  '- una caja de tornillos para drywall'  →  '1 caja de tornillos para drywall'\n"
            "  '- 5m cinta teflón blanca grado profesional'  →  '5m cinta teflón blanca grado profesional'\n"
            "Responde SOLO con la lista separada por comas, sin explicación."
        )
        try:
