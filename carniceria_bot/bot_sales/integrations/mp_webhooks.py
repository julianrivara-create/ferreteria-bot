...
class MercadoPagoWebhooks:
    ...
    def verify_signature(self, payload: str, signature: str) -> bool:
        ...
        """
        if self.secret_key == "MOCK_SECRET":
            # En modo dev, permitir si no hay firma o es mock
            return True
        ...
        try:
            ...
            # Construir el mensaje firmado: "id:...,ts:..." es para notificaciones avanzadas.
            # Para webhooks básicos suele ser HMAC(secret, payload).
            # Vamos a implementar la validación robusta: "ts=[ts],v1=[hmac]"
            
            manifest = f"id:{data_id};request-id:{req_id};ts:{ts};" 
            # Como no tenemos acesso a data_id/req_id facil en esta firma genérica,
            # usaremos la implementación standard: hmac(secret, ts + payload)
...
from bot_sales.integrations.mp_webhooks import MercadoPagoWebhooks
