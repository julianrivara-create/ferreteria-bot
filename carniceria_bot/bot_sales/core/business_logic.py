...
class BusinessLogic:
    ...
    def listar_modelos(self) -> Dict[str, Any]:
        ...
        for item in all_stock:
            ...
            if avail > 0:
                ...
                if int(item.get('storage_gb', 0) or 0) > 0:
        ...
        if success:
            ...
            if "mercadopago" in metodo_pago.lower() and sold_sku:
                try:
                    ...
                    print(f"Error generating MP link: {e}")
            ...
            if sold_sku:
                try:
                    ...
                    self.email_client.send_order_confirmation("cliente@demo.com", order_details)
                except Exception as e:
    ...
    def buscar_por_categoria(self, categoria: str) -> Dict[str, Any]:
        ...
        Args:
            categoria: Category name (iPhone, MacBook, iPad, AirPods, PlayStation)
...
¿Te gustaría avanzar con la compra?""",
            ...
            "garantia": """🛡️ GARANTÍA:
• Apple oficial: 1 año
• Nuestra garantía adicional: +3 meses
...
Para coordinar esto, te paso con la dueña.
¿Me das tu contacto?"""
        }
        
        info = policies.get(tema.lower(), "No tengo info sobre ese tema. ¿Querés que te pase con la dueña?")
    ...
    def derivar_humano(
        ...
        return {
            "status": "success",
            "lead_id": lead_id,
            "message": f"Ok, te paso con la dueña para {razon}. Te contacta pronto al {lead_contact}"
        }
    ...
    def _normalize_model(self, modelo: str) -> str:
        ...
        # REMOVED: Product-specific normalization patterns
        # Now using fuzzy matching against catalog only
