...
class PIIEncryption:
    ...
    def _init_fernet(self) -> Optional[Fernet]:
        ...
        if not encryption_key:
            ...
            password = os.getenv('ENCRYPTION_PASSWORD', 'default-insecure-password')
            salt = os.getenv('ENCRYPTION_SALT', 'default-salt').encode()
            
            if password == 'default-insecure-password':
                logging.warning("⚠️  Using default encryption password - SET ENCRYPTION_PASSWORD in production!")
