...
DB_FILE = "iphone_store_utopia.db"
LOG_PATH = "events.log"
...
PHRASES = {
    "greet": ["¡Buenas! 👋 ¿Qué iPhone estás buscando hoy?", "Hola, ¿en qué modelo estabas pensando?", "¡Hola! Decime qué buscás y me fijo el stock."],
    "ask_variant": ["Genial, ¿qué modelo exacto? (ej: iPhone 16 Pro 256GB Negro)", "Dale, decime modelo + GB + color para chequear stock."],
    ...
    "handoff": ["Mmm, para {reason} mejor hablá con la dueña. Pasame contacto y te deriva.", "Tema sensible ({reason}), lo vemos con humano. ¿WhatsApp?"],
    ...
    "policies_warranty": [
        "🛡️ Garantía:\n• Apple oficial: 1 año\n• Nuestra garantía: +3 meses adicionales\n• Cualquier tema de postventa lo maneja la dueña directamente.\n¿Tenés alguna duda específica?"
    ],
    "policies_returns": [
        "🔄 Cambios y devoluciones dependen del estado del equipo y días desde la compra. Para coordinar, necesito que hables con la dueña. ¿Pasame tu contacto?"
    ],
}
    ...
    def _init_db(self):
        ...
        if self.cursor.execute("SELECT COUNT(*) FROM stock").fetchone()[0] == 0:
            ...
            if os.path.exists(catalog_file):
                ...
                data = [
                    ("IP15-128-BLK", "iPhone 15", 128, "Black", 3, 1200),
                    ("IP15-256-BLU", "iPhone 15", 256, "Blue", 1, 1400),
                    ("IP15P-128-NAT", "iPhone 15 Pro", 128, "Natural Titanium", 2, 1600),
                    ("IP15P-256-NAT", "iPhone 15 Pro", 256, "Natural Titanium", 2, 1800),
                    ("IP15PM-256-BLK", "iPhone 15 Pro Max", 256, "Black Titanium", 1, 2000),
                ]
...
def extract_model(text: str, known_models: List[str]) -> Optional[str]:
    ...
    patterns = [
        (re.compile(r"\b(\d+)\s*pro\s*max\b", re.I), lambda m: f"iPhone {m.group(1)} Pro Max"),
        (re.compile(r"\b(\d+)\s*promax\b", re.I), lambda m: f"iPhone {m.group(1)} Pro Max"),
        (re.compile(r"\b(\d+)\s*pro\b", re.I), lambda m: f"iPhone {m.group(1)} Pro"),
        (re.compile(r"\biphone\s*(\d+)\b", re.I), lambda m: f"iPhone {m.group(1)}"),
        (re.compile(r"\b(\d{2})\b", re.I), lambda m: f"iPhone {m.group(1)}"),
    ]
            ...
            if s.model:
                ...
                if base_num:
                    ...
                    if "Pro" not in s.model:
                        pro_exists = self.db.find_matches(f"iPhone {num} Pro", None, None)
                        pro_max_exists = self.db.find_matches(f"iPhone {num} Pro Max", None, None)
                        if pro_exists or pro_max_exists:
                            msg += f"\n💎 ¿Querés subir un poco? También tengo:\n"
                            if pro_exists: msg += f"• iPhone {num} Pro (mejor cámara)\n"
                            if pro_max_exists: msg += f"• iPhone {num} Pro Max (pantalla más grande)\n"
                    
                    next_num = str(int(num) + 1)
                    newer = self.db.find_matches(f"iPhone {next_num}", None, None)
                    if newer: msg += f"\n🚀 O lo último: iPhone {next_num}\n"
